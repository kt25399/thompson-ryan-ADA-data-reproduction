...
head(Fig1C)
```
Analysis of Fig1C using student's t-test. T value is different from that reported in text (t6 = 8.57, P <
0.001) 
...
```
Analysis of Fig1D using student's t-test.
...
head(Fig1DEF_working)
```
Analysis of Fig1E using student's t-test.
...
```
### Fig1F
Analysis of Fig1F using student's t-test.

``` {r fig 1f t-test}
# t-test for LMTs 
t_test(DCX..LMTs ~ Age, data = Fig1DEF_working, var.equal = TRUE)
```
Preparing data to be plotted by calculating 
...
Preparing data to be visualized for FigS3B by pivoting each train.man column under the minute column and values in the freezing column.
...
head(Fig1I)
```
preparing data for ANOVA by pivotting shock columns to condition column and value to distance.traveled. Also factoring age and condition in addition to adding and id column
...
knitr::include_graphics("F1.large.cropped.jpg")
```
